    ...
    for file_entry in here.iterdir():
        if file_entry.is_file() and file_entry.suffix == '.csv' and '_' not in file_entry.name:
            ...
            for column in tqdm(orig_df.columns):
                if column != 'label':
                    new_df['mean_' + column] = orig_df[column].rolling(num_timewindow).mean()[::args.stepsize]
                    new_df['std_' +  column] = orig_df[column].rolling(num_timewindow).std()[::args.stepsize]
                    if(args.features == 11):
                        new_df['perc5_' + column] = orig_df[column].rolling(num_timewindow).quantile(0.05)[::args.stepsize]
                    new_df['perc25_' + column] = orig_df[column].rolling(num_timewindow).quantile(0.25)[::args.stepsize]
                    new_df['perc75_' + column] = orig_df[column].rolling(num_timewindow).quantile(0.75)[::args.stepsize]
                    if(args.features == 11):
                        new_df['perc95_' + column] = orig_df[column].rolling(num_timewindow).quantile(0.95)[::args.stepsize]
                    new_df['sumdiff_' +  column] = orig_df[column].rolling(num_timewindow).agg(getSumChanges)[::args.stepsize]
                    new_df['last_' + column] = orig_df[column].rolling(num_timewindow).agg(lambda x: np.asarray(x)[-1])[::args.stepsize]
                    if(args.features == 11):
                        new_df['min_' + column] = orig_df[column].rolling(num_timewindow).min()[::args.stepsize]
                        new_df['max_' + column] = orig_df[column].rolling(num_timewindow).max()[::args.stepsize]
                        new_df['median_' + column] = orig_df[column].rolling(num_timewindow).median()[::args.stepsize]
                else:
                    new_df['label'] = orig_df[column].rolling(num_timewindow).agg(getMostRecentFault)[::args.stepsize]
