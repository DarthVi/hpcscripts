...
from utils import str2bool, saveresults, updateboxplotsCSV, summaryboxplot
...
if __name__ == '__main__':
    ...
    for i in range(args.numiter):
        ...
        stime = time.time()
        for j in range(args.maxnumsample):
            num_train = j + 1
        ...
        #        labels = np.unique(y)
            ...
            saveresults(f_scores, keys, "/tmp/fscore_results.csv")
            saveresults(sensitivity_scores, keys, "/tmp/sensitivity_results.csv")
            saveresults(specificity_scores, keys, "/tmp/specificity_results.csv")
            saveresults(FP_scores, keys, "/tmp/fprate_results.csv")
            saveresults(FN_scores, keys, "/tmp/fnrate_results.csv")

            #for each fault, update a CSV containing the fault scores for each node and the number of nodes used as training
            for col in keys:
                updateboxplotsCSV("/tmp/fscore_results.csv", col, sumpath.joinpath("fscore"), num_train, num_iter)
                updateboxplotsCSV("/tmp/sensitivity_results.csv", col, sumpath.joinpath("sensitivity"), num_train, num_iter)
                updateboxplotsCSV("/tmp/specificity_results.csv", col, sumpath.joinpath("specificity"), num_train, num_iter)
                updateboxplotsCSV("/tmp/fprate_results.csv", col, sumpath.joinpath("FP_rate"), num_train, num_iter)
                updateboxplotsCSV("/tmp/fnrate_results.csv", col, sumpath.joinpath("FN_rate"), num_train, num_iter)

        etime = time.time()
        print("One iteration took: ", etime - stime)

    for k in keys:
        summaryboxplot(sumpath.joinpath("fscore"), k, sumpath.joinpath("fscore"), 'num_train', k + ' F1-scores')
        summaryboxplot(sumpath.joinpath("sensitivity"), k, sumpath.joinpath("sensitivity"), 'num_train', k + ' sensitivity')
        summaryboxplot(sumpath.joinpath("specificity"), k, sumpath.joinpath("specificity"), 'num_train', k + ' specificity')
        summaryboxplot(sumpath.joinpath("FP_rate"), k, sumpath.joinpath("FP_rate"), 'num_train', k + ' FP_rate')
        summaryboxplot(sumpath.joinpath("FN_rate"), k, sumpath.joinpath("FN_rate"), 'num_train', k + ' FN_rate')
