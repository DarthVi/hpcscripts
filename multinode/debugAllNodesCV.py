...
if __name__ == '__main__':
    ...
    column_indexdata = ["start_index", "end_index"]
    previous_endindex = 0
    ...
    for file_entry in tqdm(list(csvdir.iterdir())):
        if file_entry.is_file() and file_entry.suffix == '.csv':
            ...
            indexdata[nodename] = [previous_endindex, previous_endindex + len(data) - 1]
            previous_endindex = indexdata[nodename][-1] + 1
            dfs.append(data)


    #for debugging purposes
    indexdata_df = pd.DataFrame(indexdata.values(), columns=column_indexdata, index=indexdata.keys())
    indexdata_df.to_csv(resultspath.joinpath("indexdata.csv"), header=True, index=True)
    ...
    fold_indexdata = OrderedDict()
    fold_columns = ['test_min', 'test_max']
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(X), 0):
        ...
        #for debugging purposes
        fold_indexdata[fold + 1] = [np.amin(test_index), np.amax(test_index)]
    ...
    fold_indexdata_df = pd.DataFrame(fold_indexdata.values(), columns=fold_columns, index=fold_indexdata.keys())
    #for debugging purposes
    fold_indexdata_df.to_csv(resultspath.joinpath("fold_indexdata.csv"), header=True, index=True)
