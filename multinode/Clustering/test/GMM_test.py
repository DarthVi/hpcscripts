...
from clusteringApp import compute_GMM

class TestGMM(unittest.TestCase):
	@classmethod
	def setUpClass(cls)	:
		super(TestGMM, cls).setUpClass()
	...
	def testCompute_GMM(self):
		res_df = compute_GMM(TestGMM.df, 4, 'foo', 'bar', 'zoom')
		...
		for g_pred in grouped_pred:
			self.assertTrue(g_pred in TestGMM.grouped)
